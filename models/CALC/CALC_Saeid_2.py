...
def evaluate_model(cnn, ValPatch1, ValPatch2, ValLabel, w0, w1, w2):
    """
    Evaluate model performance on validation set
    
    Args:
        cnn: The network model
        ValPatch1: HSI validation patches
        ValPatch2: LiDAR validation patches
        ValLabel: Validation labels
        w0, w1, w2: Fusion weights for three levels
    ...
    """
    cnn.eval()
    pred_y = np.empty((len(ValLabel)), dtype='float32')
    number = len(ValLabel) // 100
    
    for i in range(number):
        temp1_1 = ValPatch1[i * 100:(i + 1) * 100, :, :, :].cuda()
        temp1_2 = ValPatch2[i * 100:(i + 1) * 100, :, :, :].cuda()
        temp2 = w2 * cnn(temp1_1, temp1_2)[4] + w1 * cnn(temp1_1, temp1_2)[3] + w0 * cnn(temp1_1, temp1_2)[2]
        temp3 = torch.max(temp2, 1)[1].squeeze()
        pred_y[i * 100:(i + 1) * 100] = temp3.cpu()

    if (i + 1) * 100 < len(ValLabel):
        temp1_1 = ValPatch1[(i + 1) * 100:len(ValLabel), :, :, :].cuda()
        temp1_2 = ValPatch2[(i + 1) * 100:len(ValLabel), :, :, :].cuda()
        temp2 = w2 * cnn(temp1_1, temp1_2)[4] + w1 * cnn(temp1_1, temp1_2)[3] + w0 * cnn(temp1_1, temp1_2)[2]
        temp3 = torch.max(temp2, 1)[1].squeeze()
        pred_y[(i + 1) * 100:len(ValLabel)] = temp3.cpu()

    pred_y = torch.from_numpy(pred_y).long()
    accuracy = torch.sum(pred_y == ValLabel).type(torch.FloatTensor) / ValLabel.size(0)
...
def train_network(train_loader, 
                  ...
                  l2, 
                  Classes, 
                  model_save_path='CALC_Trento.pkl',
                  eval_interval=50):
    """
    Training procedure for CALC model with adversarial learning and validation
    
    Implements the coupled adversarial training between generators and discriminators
    with multi-level feature fusion classification and validation-based model selection.
    ...
    Args:
        ...
        eval_interval: Interval for validation evaluation
    
    Returns:
        pred_y: Final predictions on test set
        val_acc: Validation accuracy history
        test_acc: Test accuracy history
    """
    # Initialize networks
    ...
    for epoch in range(EPOCH):
        for step, (b_x1, b_x2, b_y) in enumerate(train_loader):
            ...
            if step % eval_interval == 0:
                ...
                Classes_unique = np.unique(TrainLabel1)
                w0 = np.empty(len(Classes_unique), dtype='float32')
                w1 = np.empty(len(Classes_unique), dtype='float32')
                w2 = np.empty(len(Classes_unique), dtype='float32')
                
                for i in range(len(Classes_unique)):
                    cla = Classes_unique[i]
                ...
                val_accuracy, _ = evaluate_model(cnn, ValPatch1, ValPatch2, ValLabel, w0, w1, w2)
                
                # Test set evaluation
                test_accuracy, _ = evaluate_model(cnn, TestPatch1, TestPatch2, TestLabel, w0, w1, w2)
                
                print(f'Epoch: {epoch}, Step: {step} | classify loss: {ce_loss.data.cpu().numpy():.6f} '
                      f'| val accuracy: {val_accuracy:.6f} | test accuracy: {test_accuracy:.6f} '
                      f'| w0: {w0[0]:.2f} | w1: {w1[0]:.2f} | w2: {w2[0]:.2f}')
                
                val_acc.append(val_accuracy.data.cpu().numpy())
                test_acc.append(test_accuracy.data.cpu().numpy())
                class_loss.append(ce_loss.data.cpu().numpy())
                gan_loss.append(g_loss.data.cpu().numpy())
                
                # Save best model based on validation accuracy
                if val_accuracy > BestValAcc:
                    ...
                    w1B = w1
                    w2B = w2
                    #print(f'*** New best model saved! Val Acc: {BestValAcc:.6f}, Test Acc: {BestTestAcc:.6f} ***')
                
                cnn.train()  # Return to training mode

    # Final evaluation with best model
    print(f'\nLoading best model with validation accuracy: {BestValAcc:.6f}')
    ...
    Classes_unique = np.unique(TestLabel)
    EachAcc = np.empty(len(Classes_unique))
    ...
    print(f"Validation Accuracy: {BestValAcc:.4f}")
    print(f"Test OA: {OA:.4f}, AA: {AA:.4f}")
    print(f"Per-class Accuracies: {EachAcc}")
    
    return pred_y, val_acc, test_acc