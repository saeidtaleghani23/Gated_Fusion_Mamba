...
# Simplified SE-like  Channel reduction + expansion (bottleneck structure)
class BiAttn(nn.Module): #  ECA (Efficient Channel Attention)
...
# Cross-Modal Fusion Mamba Module
...
# complete Dynamic Feature Fusion Module (DFFM)
# Paper Reference: Section 3.1 & 3.3-3.4
class VSSBlock_Cross_new(nn.Module):
    def __init__(
        ...
        self.Cross_layer = Cross_layer(hidden_dim) # DFEM implementation  outputs are D₁ⁿ and D₂ⁿ
        self.self_attention_cross = SS2D_cross_new(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, **kwargs) # CMFM implementation
        self.self_attention_cross_spatial = eca_layer(channel=hidden_dim) # ECA: efficient channel attention
        self.drop_path = DropPath(drop_path)

    def forward(self, input1: torch.Tensor, input2:torch.Tensor):
        x_1 = input1.permute(0, 3, 1, 2) #(1,64,64,96)   # F₁ⁿ
        x_2 = input2.permute(0, 3, 1, 2)  # (1,64,64,96) # F₂ⁿ

        Fuse = torch.add(x_1, x_2, alpha=1) # F_fⁿ (coarse fusion)
        F_1, F_2 = self.Cross_layer(Fuse, x_1, x_2) # D₁ⁿ and D₂ⁿ
        F_1 = F_1.permute(0, 2, 3, 1)
        F_2 = F_2.permute(0, 2, 3, 1)
        Cross_x1x2 = self.self_attention_cross(self.ln_1(F_1),self.ln_2(F_2)) #(b, h, w, c) #  Linear(H₁ⁿ ⊕ H₂ⁿ)
        Cross_x1x2_ = Cross_x1x2.permute(0, 3, 1, 2) #(b, c, h, w )  Linear(H₁ⁿ ⊕ H₂ⁿ)
        Cross_x1x2_spatial =  self.self_attention_cross_spatial(Cross_x1x2_) #(b, c, h, w)  ECA(Cross_x1x2) = ECA(Linear(H₁ⁿ ⊕ H₂ⁿ))
        Cross_x1x2_spatial = Cross_x1x2_spatial.permute(0, 2, 3, 1)


        x = input2 + input1 + Cross_x1x2 + Cross_x1x2_spatial #  Final fused features
        # x = F₂ⁿ + F₁ⁿ + Linear(H₁ⁿ ⊕ H₂ⁿ) + ECA(Linear(H₁ⁿ ⊕ H₂ⁿ))
        return x

class VSSBlock_new(nn.Module): # Vision State Space Module
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
        ...
        # Paper: ESSM component
        self.op = SS2D( # ESSM Figure 2-C
            d_model=hidden_dim,
            d_state=ssm_d_state,
            ssm_ratio=ssm_ratio,
    ...
    def _forward(self, input: torch.Tensor):
        ...
        # Step 1: ESSM only (missing + F_D^l residual)
        x_ssm = self.op(x) # ESSM operation   # ESSM(LN(F_D^l)) 
        x_ = x_ssm.permute(0, 3, 1, 2)  # (b, c, h, w )
        #######
        # Step 2: ECA on ESSM output (missing LN before ECA)
        x_ = self.self_attention_cross_channel(x_)  # Efficient Channel Attention (b, c, h, w)  # ECA(x_ssm)
        x_ = x_.permute(0, 2, 3, 1)
        # MISSING: ECA(LN(Z^l))
        #####
        # Step 3: Combine ESSM + ECA
        x = x_ssm +x_   # Z^l + ECA(Z^l) approximation

        # Step 4: LDC on current features (wrong input - should be F_D^n which is input before applying normalization)
        x_conv = self.conv_branch(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)  # LDC(input)
        # WRONG: Should be LDC(F_D^n) not LDC(current_x)


        # Step 5: BiAttn instead of simple addition
        x = self.se(x_ssm) + self.se(x_conv) # BiAttn on both branches

        x = input + self.drop_path(x) # Final residual
        if self.mlp_branch:
...
#     net = VSSBlock_Cross_new(hidden_dim=96).cuda()
#     x1 = torch.rand((4, 64, 64,96)).cuda()
#     x2 = torch.rand((4, 64, 64, 96)).cuda()
#     a = net(x1, x2)
#     print(a.shape)  # Output shape: (1, 96, 64, 64)
